...
app.get('/admin/pending-images', requireAdminAuth, async (req, res) => {
    try {
        // STILL listing from Azure to see all .jpg in the container
        const blobs = await listBlobs(pendingContainerClient);
        const imageBlobs = blobs.filter(blob => blob.endsWith('.jpg'));
...
// ===========================
// Helper: Send images to a client
// ===========================
...
/**
 * Broadcast updated pending images to all admin clients
 * (This is the “global” approach if you want them to see all pending.)
 * But we also rely on each admin’s “locked” subset. 
 */
